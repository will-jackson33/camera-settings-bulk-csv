...
MUTATIONS = (
    ...
    Mutation("import.bat", "CAMERA_SETTINGS_IMPORT_BAT", IMPORT_TESTS,
             ...
             "$avoidPlain = $avoid.Clone()", "$avoidPlain = @{}"),
    Mutation("import.bat", "CAMERA_SETTINGS_IMPORT_BAT", IMPORT_TESTS,
    ...
    Mutation("import.bat", "CAMERA_SETTINGS_IMPORT_BAT", IMPORT_TESTS,
             "a changed address is sent as a static one, not with DHCP left on",
             "            $device[$dhcpAt] = Quote 'DHCPEnabled' 'False'\r\n            $dhcpAsked = 'False'\r\n"),
    Mutation("import.bat", "CAMERA_SETTINGS_IMPORT_BAT", IMPORT_TESTS,
             "an address that is not changing is never sent",
             "        if ($dhcpAsked -ne 'False') {", "        if ($false) {"),
    Mutation("import.bat", "CAMERA_SETTINGS_IMPORT_BAT", IMPORT_TESTS,
             "the cameras that move address are written last",
             "$importRuns = @(Ranges $plainNow $avoidPlain) + @(Ranges $moveNow $avoidMove)",
             "$importRuns = @(Ranges ($moveNow + $plainNow) $avoid)"),
